 ...
 * Handles three layer modes:
 *   • PRISM continuous  — gradient bar + range labels + ramp picker + Auto Adjust
 *   • Index continuous  — same as above
 *   • Index classified  — discrete class swatches (Winkler / Huglin)
 *
 * Props:
 *   isVisible        {boolean}      Any layer active?
 *   layerLabel       {string}       e.g. "Mean Temperature"
 *   unit             {string}       e.g. "°C" | "GDD" | ""
 *   isClassified     {boolean}      Show class swatches instead of gradient
 *   colormapData     {string|null}  "winkler" | "huglin" | null
 *   colormap         {string}       Active Titiler colormap id
 *   onColormapChange {Function}     (newId: string) => void
...
const ScalePanel = ({
  ...
  // Active colormap gradient CSS
  const activeCmap = CONTINUOUS_COLORMAPS.find(c => c.id === colormap)
    || CONTINUOUS_COLORMAPS[0];
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          ...
          {isClassified ? (
            <div style={{
              ...
              background: 'linear-gradient(to right, #3b82f6, #60a5fa, #86efac, #fde047, #fb923c, #ef4444, #b91c1c)',
              border: '1px solid var(--glass-border-light)', flexShrink: 0,
            }} />
          ) : (
            <div style={{
              width: '18px', height: '10px', borderRadius: '3px',
              background: activeCmap.gradient,
              border: '1px solid var(--glass-border-light)', flexShrink: 0,
            }} />
          )}
          <span style={{
      ...
      {isExpanded && (
        <div style={{ overflowY: 'auto', flex: 1, padding: '10px 12px 12px' }}>
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: '7px',
                  border: '1px solid var(--glass-border)',
                  background: showRampPicker ? 'var(--accent-medium)' : 'var(--glass-bg-light)',
                  color: showRampPicker ? 'var(--accent-text)' : 'var(--text-on-glass)',
              ...
              >
                ...
                <div style={{
                  flex: 1, height: '8px', borderRadius: '3px',
                  background: activeCmap.gradient,
                  margin: '0 4px',
                }} />
              ...
              {showRampPicker && (
                <div style={{
                  ...
                  {CONTINUOUS_COLORMAPS.map(cmap => (
                    <button
                      key={cmap.id}
                      onClick={() => { onColormapChange?.(cmap.id); setShowRampPicker(false); }}
                      title={cmap.label}
                      style={{
                        padding: '4px 6px',
                        borderRadius: '6px',
                        border: cmap.id === colormap
                          ? '1.5px solid var(--accent-border)'
                          : '1px solid var(--glass-border-light)',
                        background: cmap.id === colormap ? 'var(--accent-dim)' : 'transparent',
                        cursor: 'pointer',
                    ...
                    >
                      ...
                      <span style={{
                        ...
                        color: cmap.id === colormap ? 'var(--accent-text)' : 'var(--text-on-glass-dim)',
              ...
              {showAutoAdjust && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <button
                    title="Auto Adjust calculates the color scale from data in the current viewport using the 2nd and 98th percentiles (p2–p98)."
                    style={{
          ...
          {/* ── Classified mode — discrete class swatches ── */}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {classEntries.map(entry => (
                <div key={entry.key} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{
                    fontSize: '10px', color: 'var(--text-on-glass)',
